...
% recz = [recz3-0.5];
%(1=Ex, 2=Ez)
srctype = 1*ones(size(srcz));
rectype = 1*ones(size(recz));
...
signal = ricker(Hz, t, center);
xsrcpulse = repmat(signal,nsrc,1);
zsrcpulse = repmat(signal,nsrc,1)*0;
figure()
subplot(3,1,1)
plot(t, signal)
subplot(3,1,2)
